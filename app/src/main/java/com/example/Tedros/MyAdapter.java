...
public class MyAdapter extends RecyclerView.Adapter<MyHolder> implements Filterable {
    ...
    public MyHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.jar_list,null);
        return new MyHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final MyHolder holder, @SuppressLint("RecyclerView") final int position) {
        holder.mTitle.setText(models.get(position).getJartype());
        holder.mCost.setText("₱"+models.get(position).getCost()+".00");
        byte[] decodedString = Base64.decode(models.get(position).getImage(), Base64.DEFAULT);
    ...
    Filter filter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence charSequence) {
            ...
            if(charSequence.toString().isEmpty()){
                ...
                for(Model item : list){
                    if(item.getJartype().toLowerCase().startsWith(charSequence.toString().toLowerCase())){
                        filterlist.add(item);
                    }
                }
            }
